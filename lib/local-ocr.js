...
function runOcr(kind, base64, filename) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', () => {
      ...
      if (res.ok) return done(resolve, res.text);
    ...
    try {
      child.stdin.write(JSON.stringify({ kind, base64, filename }));
      child.stdin.end();
    } catch (e) {
      try { child.kill('SIGKILL'); } catch (_) {}
...
async function ocrImage(base64, filename = 'image') {
  return runOcr('image', base64, filename);
}

async function ocrPdf(base64, filename = 'document') {
  return runOcr('pdf', base64, filename);
}
