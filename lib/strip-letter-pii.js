...
const { PII_ORG_WORDS, findFooterNameTokens, findDependentNameTokens, redactFusedTokens, redactPersonalPhones, redactPersonalEmails } = require('./va-pii-rules');
...
function stripLetterPII(s) {
  ...
  // Numeric identifiers.
  // ICN / participant ID FIRST, before even the fused-token pass: OCR can
  // split the number around the V ("123456789V 123456", found 2026-08-29 via
  // the desktop redactor's adversarial round trip), and any earlier digit
  // rule (fused pass, bare-SSN) would half-consume the leading nine digits
  // and leave the tail readable. Consuming the whole spaced form here is the
  // same no-half-consumption principle the fused pass exists for.
  out = out.replace(/\b\d{9,11}\s?V\s?\d{5,7}\b/g, '[ID REDACTED]');
  out = out.replace(/\b(ICN|integrated control number)[:#\s]*[\dV]{6,}(?:\s\d{5,7}\b)?/gi, '$1: [ID REDACTED]');
  // Fused-token pass: catches OCR-glued labels/digits a \b anchor
  ...
  // Personal contact details (2026-08-29, owner default 2): unambiguous
  // 10-digit phone shapes except the VA's public toll-free lines, and
  // emails except agency addresses at va.gov. Runs AFTER the SSN/file
  // rules so a labeled identifier is never half-eaten as a phone.
  out = redactPersonalPhones(out, '[PHONE REDACTED]');
  out = redactPersonalEmails(out, '[EMAIL REDACTED]');
