...
function findFooterNameTokens(text) {
  ...
  const addName = (t) => { if (t && t.length >= 3 && !PII_ORG_WORDS.has(t.toUpperCase())) nameTokens.add(t); };
  let m;
  const footerComma = /\b([A-Z][A-Za-z'\-]{2,})\s*,\s+([A-Z][A-Za-z'\-]{1,})(?:\s+[A-Z]\b\.?)?(?=\s*(?:ICN\b|Page\b|\d+\s+of\s+\d+))/g;
  while ((m = footerComma.exec(text)) !== null) { addName(m[1]); addName(m[2]); }
  const footerPlain = /\b([A-Z][A-Za-z'\-]{2,})\s+([A-Z][A-Za-z'\-]{2,})(?=\s+\d+\s+of\s+\d+)/g;
